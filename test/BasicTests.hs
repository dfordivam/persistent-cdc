...
specs = describe "persistent" $ do
  ...
  -- Not implemented in 2.6, the tests are from github HEAD
  -- it "getEntity" $ db $ do
  --     Entity k p <- insertEntity $ Person "name" 1 Nothing
...
{- FIXME
  ...
  it "rawSql/entity" $ db $ do
      ...
      let query = T.concat [ "SELECT ??, ?? "
                           ...
                           , "AND ", escape "Pet", ".", escape "owner_id", " = "
                                   , person, ".", escape "id"
  ...
  -- XXX - ReverseField not added yet in CDC
  ...
  --   (caseCommitRollback >> runResourceT (runConn cleanDB))
  --   return ()
...
#ifndef WITH_NOSQL
  -- XXX - NoPrefix stuff not added in CDC
